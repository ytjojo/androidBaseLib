...
public class Thumbnail
{
     ...
     * @return Thumbnail object
     * @throws IOException 画像ファイルにアクセスできなかったときに投げられる
     * @throws FileNotFoundException 画像ファイルが存在しなかったときに投げられる
     */
    ...
    public Thumbnail centerCrop(int width, int height, int backgroundColor)
    {
        ...
        if (bitmapWidth > canvasWidth || bitmapHeight > canvasHeight) {
            ...
            Bitmap.Config config = (canvasWidth == width && canvasHeight == height &&
                    mBitmap.getConfig() != null) ? mBitmap.getConfig() : Bitmap.Config.ARGB_8888;
