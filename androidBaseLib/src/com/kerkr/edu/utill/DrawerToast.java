...
import com.ytjojo.androidbaselib.R;



/**
 * 抽屉吐司 通过getInstance(context)获得本类的单例
 */
public class DrawerToast {
    ...
    // 入场动画持续时间
    private static final int TIME_START_ANIM = 150;

    // 离场动画持续时间
    private static final int TIME_END_ANIM = 100;

    // 每条Toast显示持续时间
    private static final int TIME_DURATION = 1200;

    private static DrawerToast instance;

    public static DrawerToast getInstance(Context context) {
        if (instance == null) {
            instance = new DrawerToast(context.getApplicationContext());
        }
        return instance;
    }
    public static DrawerToast getInstance() {
        if (instance == null) {
            instance = new DrawerToast(null);
        }
        return instance;
    }
    ...
    private void initView() {
        ...
        mTopView.setGravity(Gravity.CENTER_HORIZONTAL | Gravity.BOTTOM);

        View gapView = new View(mContext);
        gapView.setLayoutParams(new LayoutParams(screenWidth, screenHeight / 8));
        mTopView.addView(gapView);
    ...
    public final void show(final View v, Long duration, Animation startAnim, final Animation endAnim) {
        ...
        if (viewCount == 2) {//第2个View是空白gapView
            mTopView.removeViewAt(0);
        }
        showToast();
        mTopView.addView(v, 0);
        // 获得入场动画
        if (startAnim == null)
    ...
    public final void hide(final View v, Animation endAnim) {
        if (v == null || mTopView.indexOfChild(v) < 0)
            return;
        ...
        mHandler.postDelayed(new Runnable() {
            ...
            public void run() {
                if (v == null || mTopView.indexOfChild(v) < 0)
                    return;
                // 移除指定控件
                mTopView.removeView(v);
                // 隐藏顶层容器控件
                if (mTopView.getChildCount() == 1)
                    hideToast();
            }
        }, TIME_END_ANIM);
    }
    ...
    public TextView getTextView(String msg) {
        ...
        CompatUtil.setBackground(tv,mContext.getResources().getDrawable(R.drawable.shape_toast));
    ...
    protected Animation getStartAnimation() {
        ...
        TranslateAnimation animTrans = new TranslateAnimation(Animation.RELATIVE_TO_PARENT, 0f, Animation.RELATIVE_TO_PARENT, 0f,
    ...
    protected Animation getEndAnimation() {
        ...
        TranslateAnimation animTrans = new TranslateAnimation(Animation.RELATIVE_TO_PARENT, 0f, Animation.RELATIVE_TO_PARENT, 0f,
                Animation.RELATIVE_TO_PARENT, 0f, Animation.RELATIVE_TO_PARENT, -0.05f);
    ...
    private void initTN() {
        ...
        try {
            ...
            mObj = mTN.get(mToast);
            showMethod = mObj.getClass().getDeclaredMethod("show", new Class[]{});
            hideMethod = mObj.getClass().getDeclaredMethod("hide", new Class[]{});
