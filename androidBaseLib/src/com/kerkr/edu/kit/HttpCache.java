...
import com.kerkr.edu.compatible.LruMemoryCache;
...
public class HttpCache
{
	...
	public void put(String url, String result, long expiry)
	{
		...
		mMemoryCache.put(url, result,SystemClock.uptimeMillis() + expiry);
	}
	...
	public boolean isEnabled(int method)
	{
		if (method <-1 || method >7) return false;

		Boolean enabled = httpMethod_enabled_map.get(method);
		return enabled == null ? false : enabled;
	}


	public void setEnabled(int method, boolean enabled)
	{
		httpMethod_enabled_map.put(method, enabled);
	}

	private final static ConcurrentHashMap<Integer, Boolean> httpMethod_enabled_map;

	static
	{
		httpMethod_enabled_map = new ConcurrentHashMap<Integer, Boolean>(10);
		httpMethod_enabled_map.put(Method.GET, true);
	}
}
