...
public class BlurBehind {
	...
	private static final LruCache<String, Bitmap> mImageCache = new LruCache<String, Bitmap>(
			1);
	...
	public Bitmap initBitmap(View originalView, View overlayView) {
		this.mOriginalView = originalView;
		int[] overlayViewLocation = new int[2];
		int[] originalViewLocation = new int[2];
		overlayView.getLocationOnScreen(overlayViewLocation);
		originalView.getLocationOnScreen(originalViewLocation);
		int translateX = overlayViewLocation[0] - originalViewLocation[0];
		int translateY = overlayViewLocation[1] - originalViewLocation[1];
		mSrcBitmap = BlurUtill.drawViewToBitmap(mOriginalView,
				overlayView.getMeasuredWidth(),
				overlayView.getMeasuredHeight(), translateX, translateY, mDownSampling);
		return mSrcBitmap;
	}
	...
	public void execute(Activity activity,
		...
		if (mState.equals(State.READY)) {
			...
			cacheBlurBehindAndExecuteTask = new CacheBlurBehindAndExecuteTask(
					activity, onBlurCompleteListener);
	...
	public void setBackground(Activity activity) {
		if (mImageCache.size() != 0) {
			BitmapDrawable bd = new BitmapDrawable(activity.getResources(),
					mImageCache.get(KEY_CACHE_BLURRED_BACKGROUND_IMAGE));
			bd.setAlpha(mAlpha);
			if (mFilterColor != -1) {
	...
	private class CacheBlurBehindAndExecuteTask extends
		...
		protected Void doInBackground(Void... params) {
			Bitmap blurredBitmap = FastBlur.doBlurRenderScript(activity, image,
					CONSTANT_BLUR_RADIUS, false);
