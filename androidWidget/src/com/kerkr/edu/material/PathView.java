...
public class PathView extends View {
     ...
     */
    private final SvgHelper SvgHelper = new SvgHelper(paint);
    /**
    ...
    public void setPaths(final List<Path> paths) {
        for (Path path : paths) {
            this.paths.add(new SvgHelper.SvgPath(path, paint));
        }
    ...
    public void setPath(final Path path) {
        paths.add(new SvgHelper.SvgPath(path, paint));
        synchronized (mSvgLock) {
            updatePathsPhaseLocked();
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        synchronized (mSvgLock) {
            ...
            final int count = paths.size();
            for (int i = 0; i < count; i++) {
                final SvgHelper.SvgPath svgPath = paths.get(i);
    ...
    private void fillAfter(final Canvas canvas) {
        if (svgResourceId != 0 && fillAfter && progress == 1f) {
            SvgHelper.drawSvgAfter(canvas, width, height);
        }
    }
        ...
        if (svgResourceId != 0) {
            mLoader = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    SvgHelper.load(getContext(), svgResourceId);

                    synchronized (mSvgLock) {
                        width = w - getPaddingLeft() - getPaddingRight();
                        height = h - getPaddingTop() - getPaddingBottom();
                        paths = SvgHelper.getPathsForViewport(width, height);
                        updatePathsPhaseLocked();
                    }
                }
            }, "SVG Loader");
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (SvgHelper.SvgPath path : paths) {
