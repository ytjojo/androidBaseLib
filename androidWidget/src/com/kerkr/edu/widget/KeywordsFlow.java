 ...
 *         自定义 搜索 关键词的 视图 很不错的 效果 addKeyword（）方法 加入 关键词 go2show 方法来 开始 动画
 * 
 * 
 */
public class KeywordsFlow extends FrameLayout implements OnGlobalLayoutListener
{
	...
	private static Interpolator interpolator; // 窜改器
	...
	private void init()
	{
		...
		getViewTreeObserver().addOnGlobalLayoutListener(this); // 视图树观察者
																// 添加全局Layout监听
		interpolator = AnimationUtils.loadInterpolator(getContext(), android.R.anim.decelerate_interpolator);
		animAlpha2Opaque = new AlphaAnimation(0.0f, 1.0f);// 由透明至不透明
		animAlpha2Transparent = new AlphaAnimation(1.0f, 0.0f); // 由不透明至透明
		animScaleLarge2Normal = new ScaleAnimation(2, 1, 2, 1); // 由屏幕外部至正常
		animScaleNormal2Large = new ScaleAnimation(1, 2, 1, 2); // 由正常至屏幕外部
		animScaleZero2Normal = new ScaleAnimation(0, 1, 0, 1); // 由屏幕中心至正常
		animScaleNormal2Zero = new ScaleAnimation(1, 0, 1, 0); // 由正常至屏幕中心
	}
	...
	private void disapper()
	{
		...
		for (int i = size - 1; i >= 0; i--)
		{
			...
			Log.i("INFO", txt.getText() + " leftM=" + layParams.leftMargin + " topM=" + layParams.topMargin + " width=" + txt.getWidth());
...
//				Log.e("Search", "ListX:" + (i * xItem) + "#listY:" + (i * yItem + (yItem >> 2)));
			...
			for (int i = 0; i < size; i++)
			{
				...
				else if (xy[IDX_X] == 0) xy[IDX_X] = Math.max(random.nextInt(xItem), xItem / 3);
				xy[IDX_DIS_Y] = Math.abs(xy[IDX_Y] - yCenter);
				txt.setTag(xy);
				if (xy[IDX_Y] > yCenter) listTxtBottom.add(txt);
	...
	public AnimationSet getAnimationSet(int[] xy, int xCenter, int yCenter, int type)
	{
		...
		if (type == OUTSIDE_TO_LOCATION)
		{
			...
			TranslateAnimation translate = new TranslateAnimation((xy[IDX_X] + (xy[IDX_TXT_LENGTH] >> 1) - xCenter) << 1, 0,
		...
		else if (type == LOCATION_TO_OUTSIDE)
		{
			...
			TranslateAnimation translate = new TranslateAnimation(0, (xy[IDX_X] + (xy[IDX_TXT_LENGTH] >> 1) - xCenter) << 1, 0,
					(xy[IDX_Y] - yCenter) << 1);
			animSet.addAnimation(translate);
		}
