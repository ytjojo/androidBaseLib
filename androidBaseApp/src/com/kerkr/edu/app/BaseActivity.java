...
public abstract class BaseActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setupRootFragment(savedInstanceState);
    ...
    private void setupRootFragment(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            ...
            if (mRootFragment != null) {
                getSupportFragmentManager().beginTransaction().add(R.id.fragment_Container, mRootFragment).commit();
            }
        }
        else {
            mRootFragment = (BaseFragment) getSupportFragmentManager().getFragment(savedInstanceState, getFragmentTag(mRootFragment.getClass()));
            if (mRootFragment == null) {
                return;
            }
            
            if (mRootFragment.isAdded()) {
                getSupportFragmentManager().beginTransaction().show(mRootFragment).commit();
            }
            else {
                getSupportFragmentManager().beginTransaction().add(R.id.fragment_Container, mRootFragment).commit();
    ...
    public View getView(String idString) {
        ...
        int titleId = Resources.getSystem().getIdentifier(idString, "id", "android");
        if (titleId < 0) {
            return null;
        }
    ...
    public void pushFragmentToBackStack(BaseFragment fragment, int containerId) {
        if (!isActive() || fragment == null) {
            return;
        }
        //        Fragment addedFragment = getSupportFragmentManager().findFragmentByTag(getFragmentTag(fragment.getClass()));
    ...
    public void startFragment(BaseFragment fragment) {
        ...
        pushFragmentToBackStack(fragment, R.id.fragment_Container);
    }
    ...
    public void popBackFragent() {
        ...
        VALog.i("  popbackFragment" + size + "   : " + fm.getFragments().size());
        if (size == 0) {
            
            finish();//当是最初的fragment时候调用这个方法会退出
            return;
        }
        fm.popBackStack();
//     
...
//        ft.commitAllowingStateLoss();
    ...
    public void pushFragmentToBackStack(int containerId, Class<? extends BaseFragment> cls) {
        if (cls == null) {
            return;
        }
        try {
            ...
            pushFragmentToBackStack(fragment, containerId);
        }
    ...
    public void launch(BaseActivity activity, View transitionView, String url, Class clazz) {
     ...
     * 界面颜色的更改
     */
    public void colorChange(Bitmap bitmap, final PagerSlidingTabStrip pagerSlidingTabStrip) {
        // Palette的部分
        Palette.generateAsync(bitmap, new Palette.PaletteAsyncListener() {
            ...
            public void onGenerated(Palette palette) {
                ...
                if (pagerSlidingTabStrip != null) {
                    ...
                    pagerSlidingTabStrip.setTextColor(vibrant.getTitleTextColor());
                    // 其中状态栏、游标、底部导航栏的颜色需要加深一下，也可以不加，具体情况在代码之后说明
                    pagerSlidingTabStrip.setIndicatorColor(PaletteUtils.colorBurn(vibrant.getRgb()));
                }
                ...
                if (android.os.Build.VERSION.SDK_INT >= 21) {
                    ...
                    // 很明显，这两货是新API才有的。
                    window.setStatusBarColor(PaletteUtils.colorBurn(vibrant.getRgb()));
                    window.setNavigationBarColor(PaletteUtils.colorBurn(vibrant.getRgb()));
                }
            }
        });
    }
    ...
    public void setContentViewInternal(int layoutResID) {
        if (layoutResID == R.layout.base_fragment) {
            ...
            View v = LayoutInflater.from(this).inflate(layoutResID, mWorkSpaceView, false);
     ...
     * @param params
     */
    public void setContentViewInternal(View view, android.view.ViewGroup.LayoutParams params) {
    ...
    public void setOverlay(boolean b) {
        FrameLayout.LayoutParams params = (LayoutParams) mWorkSpaceView.getLayoutParams();
        if (params != null) {
            if (b) {
    ...
    protected void hideProgressDialog() {
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.JELLY_BEAN && !isActive()) {
            return;
        }
        if (progressDialog != null) {
    ...
    public void showProgressBar() {
        ...
        progressView.findViewById(R.id.dialog_loading_title).setVisibility(View.VISIBLE);
    ...
    public DisplayImageOptions getDisplayImageOptions(int defaultDrawable, boolean iscacheOnDisk, int corner) {
    ...
    protected void handleNotificationIntent(Intent intent) {
        if (intent != null && umengNotificationListener != null) {
            ...
            if (bd != null && bd.containsKey(Constans.NOTIFY_TYPE)) {
                ...
                umengNotificationListener.onNotificationIntentReceived(notifyType, value, false);
            }
        }
    }
    ...
    public interface UmengNotificationListener {
        //    通知跳转页面的intent获得后
        void onNotificationIntentReceived(String notifyType, String value, boolean needDelete);
    }
