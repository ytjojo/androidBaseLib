...
import com.kerkr.edu.reflect.CollectionUtils;
...
public class FragmentStack implements OnBackStackChangedListener {
    ...
    public void setupRoot(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            ...
            mRootFragment = (BaseFragment) Fragment.instantiate(mActivity, mRootFragmentClass.getName(), savedInstanceState);
            if (mRootFragment != null) {
                mFragmentManager.beginTransaction().replace(mContainerId, mRootFragment).commit();
            }
        }
        else {
            mRootFragment = (BaseFragment) mFragmentManager.getFragment(savedInstanceState, getFragmentTag(mRootFragment.getClass()));
            if (mRootFragment == null) {
                return;
            }
            
            if (mRootFragment.isAdded()) {
                mFragmentManager.beginTransaction().show(mRootFragment).commit();
            }
            else {
                mFragmentManager.beginTransaction().replace(mContainerId, mRootFragment).commit();
    ...
    public void popToFragment(Class<?> cls) {
        ...
        BaseFragment fragment = (BaseFragment) mFragmentManager.findFragmentByTag(cls.toString());
    ...
    public BaseFragment createRootFragment(Class<? extends BaseFragment> clazz, Bundle args) {
        return (BaseFragment) Fragment.instantiate(mActivity, mRootFragment.getClass().getName(), args);
    }
    ...
    public Fragment getActiveFragment() {
        ...
        String tag = mFragmentManager.getBackStackEntryAt(mFragmentManager.getBackStackEntryCount() - 1).getName();
    ...
    public void setRootFragment(final BaseFragment startFragment) {
        ...
        mRootFragment = startFragment;
        mRootFragmentClass = startFragment.getClass();
    ...
    private void replaceFragment(final Fragment fragment) {
        mFragmentManager.beginTransaction().replace(mContainerId, fragment, getName(fragment)).commit();
        mFragmentManager.executePendingTransactions();
        //        mFragmentStack.add(fragment);
    }
    
    private String getName(Fragment fragment) {
        StringBuilder sb = new StringBuilder(fragment.getClass().toString());
        return sb.toString();
    }
